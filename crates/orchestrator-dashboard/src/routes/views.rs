//! HTML view + SSE stream handlers for the dashboard. Extracted from
//! `main.rs` so the binary's main module is a thin composition root.
...
use orchestrator_types::{
    brief::EscalationMode, role::McpServer, AgentRole, MessageEdge, PackageManager, PermitScope,
    Project, ProjectSlug, RoleName, RoleRef, StandingOrders, SubstrateClass, TeamName,
...
pub async fn brief_detail(Path(id): Path<String>) -> Html<String> {
    let body = format!(
        r#"
...
pub async fn roles_list(State(state): State<AppState>) -> Result<Html<String>, AppError> {
...
pub async fn role_new_form() -> Html<String> {
...
pub async fn teams_list(State(state): State<AppState>) -> Result<Html<String>, AppError> {
...
pub async fn team_new_form() -> Html<String> {
...
pub async fn projects_list(State(state): State<AppState>) -> Result<Html<String>, AppError> {
...
pub async fn project_new_form() -> Html<String> {
